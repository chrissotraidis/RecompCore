#include "slippi-native-diagnostics.hpp"
...
std::string ConvertConnectCodeForGame(const std::string& input)
{
  ...
  connect_code.resize(SLIPPI_CONNECT_CODE_LENGTH + 2);
  return connect_code;
}
    ...
    : IEXIDevice(system)
{
  ...
  ++SlippiCompat::device_creations;

  std::string user_config_folder = SlippiCompat::UserDirectory();
  ...
  slprs_exi_config.scm_slippi_semver_str = SlippiCompat::Version().c_str();
  ...
  game_file_loader = std::make_unique<SlippiGameFileLoader>(resource_directory);
...
void CEXISlippi::createNewFile()
{
  ...
  if (dir_path.empty())
  {
    Config::SetCurrent(Config::SLIPPI_REPLAY_DIR, File::GetUserPath(D_USER_IDX) + "Slippi/Replays");
...
void CEXISlippi::prepareGameInfo(u8* payload)
{
  ...
  Config::SetCurrent(Config::MAIN_OVERCLOCK_ENABLE, g_playback_status->orig_OC_enable);
  Config::SetCurrent(Config::MAIN_OVERCLOCK, g_playback_status->orig_OC_factor);
...
void CEXISlippi::handleOnlineInputs(u8* payload)
{
  ...
  if (isDisconnected())
  {
    ...
    {
      OSD::AddTypedMessage(
          OSD::MessageType::SlippiPoorPerformanceTermination,
...
bool CEXISlippi::shouldAdvanceOnlineFrame(s32 frame)
{
  ...
  if (is_time_sync_frame)
  {
        ...
        last_search.mode != SlippiMatchmaking::OnlinePlayMode::RANKED)
    {
      ...
      // message.
      OSD::AddTypedMessage(
          OSD::MessageType::SlippiPerformanceWarning,
...
void CEXISlippi::startFindMatch(u8* payload)
{
  ...
  if (!SlippiDirectProbe::AllowsSearch(static_cast<int>(search.mode), shift_jis_code)) {
  ...
  else if (search.mode == SlippiMatchmaking::OnlinePlayMode::TEAMS)
  {
    ...
    if (local_selections.character_id >= 26 &&
        SlippiCompat::RuntimeConfig().melee_version != Melee::Version::MEX)
    {
      forced_error = "The character you selected is not allowed in this mode";
      return;
    }
...
void CEXISlippi::prepareOnlineMatchState()
{
  ...
  if (local_player_ready && remote_players_ready)
  {
    ...
    {
      auto is_MEX = SlippiCompat::RuntimeConfig().melee_version == Melee::Version::MEX;
  ...
  *item_bits = Common::swap64(new_items_value);
  ...
  local_player_name = ConvertStringForGame(local_player_name, SLIPPI_MAX_NAME_LENGTH);
...
#endif
    name = ConvertStringForGame(name, SLIPPI_MAX_NAME_LENGTH);
  ...
  auto chars_per_ame = (SLIPPI_MAX_NAME_LENGTH - (num_opponents - 1)) / num_opponents;
  std::string opp_text = "";
  ...
  opp_name = ConvertStringForGame(opp_text, SLIPPI_MAX_NAME_LENGTH);
...
void CEXISlippi::prepareFileLength(u8* payload)
{
  ...
  std::fprintf(stderr, "[slippi-probe] resource %s bytes=%u\n", file_name.c_str(), size);
...
void CEXISlippi::prepareGctLoad(u8* payload)
{
  ...
  // Keep the actual heap allocation supplied by this boot. No fixed address.
  auto& memory = Core::System::GetInstance().GetMemory();
  if (address >= 0x80000000u && address - 0x80000000u < memory.GetRamSizeReal() &&
      gct.size() <= memory.GetRamSizeReal() - (address - 0x80000000u))
  {
    SlippiCompat::loaded_gct_address = address;
    SlippiCompat::loaded_gct_size = static_cast<u32>(gct.size());
  }
  else
  {
    SlippiCompat::loaded_gct_address = 0;
    SlippiCompat::loaded_gct_size = 0;
  }
...
void CEXISlippi::prepareOnlineStatus()
{
  ...
  if (is_logged_in)
  {
    ...
    version::Semver200_version current_version(SlippiCompat::Version());
  ...
  std::string player_name = ConvertStringForGame(user_info.display_name, SLIPPI_MAX_NAME_LENGTH);
...
void CEXISlippi::handleGetPlayerSettings()
{
  ...
  for (int i = 0; i < 4; i++)
  {
    ...
    for (int j = 0; j < 16; j++)
    {
      auto str = ConvertStringForGame(messages_by_player[i][j], SLIPPI_MAX_MESSAGE_LENGTH);
...
void CEXISlippi::DMAWrite(u32 _uAddr, u32 _uSize)
{
  ...
  {
    ++SlippiCompat::command_counts[byte];
  ...
  while (buf_loc < _uSize)
  {
    ...
    if ((byte == CMD_RECEIVE_POST_FRAME_UPDATE || byte == CMD_RECEIVE_INITIAL_RNG ||
         byte == CMD_RECEIVE_ITEM || byte == CMD_RECEIVE_GAME_INFO ||
         byte == CMD_RECEIVE_GAME_END) && SlippiCompat::game_frame_observer &&
        payload_len < _uSize - buf_loc)
      SlippiCompat::game_frame_observer(byte, &mem_ptr[buf_loc], payload_len + 1);
    switch (byte)
    {
    ...
    case CMD_FRAME_BOOKEND:
      ...
      if (slippi_netplay)
        slprs_exi_device_reporter_push_replay_data(slprs_exi_device_ptr, &mem_ptr[buf_loc],
                                                   payload_len + 1);
      if (SlippiCompat::game_frame_observer && payload_len < _uSize - buf_loc)
        SlippiCompat::game_frame_observer(byte, &mem_ptr[buf_loc], payload_len + 1);
      break;
