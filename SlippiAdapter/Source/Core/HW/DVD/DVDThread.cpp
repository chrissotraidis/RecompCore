...
// Slippi resource reads run on the CPU thread and wait for outstanding DVD work.
bool DVD::DVDThread::ReadFile(const std::string& file_name, std::vector<u8>& buffer)
{
  buffer.clear();
  if (!HasDisc()) return false;
  WaitUntilIdle();
  const auto* filesystem = m_disc->GetFileSystem(DiscIO::PARTITION_NONE);
  if (!filesystem) return false;
  auto info = filesystem->FindFileInfo(file_name);
  if (!info || info->IsDirectory()) return false;
  buffer.resize(info->GetSize());
  if (DiscIO::ReadFile(*m_disc, DiscIO::PARTITION_NONE, info.get(), buffer.data(), buffer.size()) != buffer.size())
  {
    buffer.clear();
    return false;
  }
