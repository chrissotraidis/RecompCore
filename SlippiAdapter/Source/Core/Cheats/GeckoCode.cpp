...
static Installation InstallCodeHandlerLocked(const Core::CPUThreadGuard& guard)
{
  ...
  if (SlippiCompat::boot_enabled)
  {
    // Pinned Slippi bootloader semantics: avoid the USB Gecko EXI conflict,
    // then load the full GCT into the game's heap through Slippi EXI.
    PowerPC::MMU::HostWrite<u32>(guard, 0x4e800020, 0x80001d6c);
    std::string bootloader;
    if (!File::ReadFileToString(File::GetSysDirectory() + "bootloader.gct", bootloader) ||
        bootloader.size() > codelist_end_address - codelist_base_address)
      return Installation::Failed;
    for (size_t i = 0; i < bootloader.size(); ++i)
      PowerPC::MMU::HostWrite<u8>(guard, bootloader[i], static_cast<u32>(codelist_base_address + i));
    PowerPC::MMU::HostWrite<u32>(guard, 0, HLE_TRAMPOLINE_ADDRESS);
    std::fprintf(stderr, "[slippi-probe] installed bootloader bytes=%zu\n", bootloader.size());
  }
  else
  {
