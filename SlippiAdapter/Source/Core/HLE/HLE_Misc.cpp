...
{
  auto& system = guard.GetSystem();
  // The Slippi code handler relocates C2 instructions in its heap GCT. It does
  // not consistently invalidate that destination. Retire cached verdicts on
  // handler return; StaticRecomp must still verify live bytes on next entry.
  if (SlippiCompat::boot_enabled && SlippiCompat::loaded_gct_size)
  {
    system.GetJitInterface().InvalidateICache(SlippiCompat::loaded_gct_address,
                                             SlippiCompat::loaded_gct_size, true);
    ++SlippiCompat::gct_handler_invalidations;
  }
