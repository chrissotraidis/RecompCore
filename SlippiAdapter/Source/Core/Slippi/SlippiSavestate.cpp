...
void SlippiSavestate::Load(std::vector<PreserveBlock> blocks)
{
  ...
  for (auto it = backup_locs.begin(); it != backup_locs.end(); ++it)
  {
    ...
    Core::System::GetInstance().GetJitInterface().InvalidateICache(it->start_address, size, true);
  }
  ...
  for (auto it = blocks.begin(); it != blocks.end(); ++it)
  {
    ...
    Core::System::GetInstance().GetJitInterface().InvalidateICache(it->address, it->length, true);
  }
}
