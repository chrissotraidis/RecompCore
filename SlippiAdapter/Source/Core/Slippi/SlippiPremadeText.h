...
inline std::u32string SlippiUTF8ToUTF32(std::string_view input)
{
  const auto units = UTF8ToUTF16(input);
  std::u32string result;
  for (size_t i = 0; i < units.size(); ++i)
  {
    char32_t value = units[i];
    if (value >= 0xd800 && value <= 0xdbff && i + 1 < units.size() &&
...
public:
  ...
  std::vector<u8> GetPremadeTextData(u8 textId, ...)
  {
    ...
    for (int m = 0; m < matches.size(); m++)
    {
      ...
      auto utfMatch = SlippiUTF8ToUTF32(firstMatch);
