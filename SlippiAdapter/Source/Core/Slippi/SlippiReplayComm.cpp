...
SlippiReplayComm::SlippiReplayComm()
{
  INFO_LOG_FMT(EXPANSIONINTERFACE, "SlippiReplayComm: Using playback config path: {}",
               SlippiCompat::RuntimeConfig().slippi_input);
  m_config_file_path = SlippiCompat::RuntimeConfig().slippi_input;
}
...
void SlippiReplayComm::loadFile()
{
  ...
  u64 mod_time = SlippiCompat::FileModificationTime(m_config_file_path);
