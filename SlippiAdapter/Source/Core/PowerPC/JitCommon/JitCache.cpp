...
void JitBaseBlockCache::Clear()
{
  ...
  // StaticRecomp has no JIT blocks. Its cached instruction-hash verdicts must
  // follow this same cache-clear event (including Gecko's ICache reset).
  // Use the existing virtual invalidation path; every chunk still verifies
  // its live bytes before native execution. No generated code is freed here.
  if (dynamic_cast<StaticRecompCore*>(&m_jit))
    InvalidateICacheInternal(0, 0, 0xffffffffu, true);
}
