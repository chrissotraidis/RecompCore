...
{
  if (this == &m_mixer->m_dma_mixer && num_samples > 2048)
  {
...
{
  u32 rate_divisor = m_input_sample_rate_divisor;
  p.Do(rate_divisor);
  if (p.IsReadMode())
  {
...
void Mixer::MixerFifo::ApplyGranuleWindow(Granule* output, const Granule& input,
                                             std::size_t start)
{
  // import numpy as np
...
void Mixer::MixerFifo::Enqueue()
{
  ...
  ApplyGranuleWindow(&m_queue[head], m_next_buffer, m_next_buffer_index);
